...
add_row = (first) ->
  ...
  $('#discount-line-items').append(rendered)
  #
  ...
  #
  # render the second part of the product line (search columns[s])
  # and insert into product slot made within layout above
  #
  compiled = _.template(_product_column_tpl)
  rendered =  compiled
      row: row_id

  $("#products-"+row_id).html(rendered)
  ...
  $('#autocomplete-input-category-'+row_id).bind 'input', ->
    ...
    return
  $('#autocomplete-input-product-'+row_id).bind 'input', ->
    window.checkModelData this
    return
